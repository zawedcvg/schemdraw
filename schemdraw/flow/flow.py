...
class Box(Element):
    ...
    def _set_segments(self):
        ...
        self.segments.append(
            Segment([(0, 0), (0, h / 2), (w, h / 2), (w, -h / 2), (0, -h / 2), (0, 0)])
        )
...
class RoundBox(Box):
    ...
    _element_defaults = {"cornerradius": 0.3}
    ...
    def _set_anchors(self):
        ...
        self.anchors["NNE"] = (
            3 * w / 4 if cornerradius < w / 4 else w - cornerradius,
            h / 2,
        )
        ...
        self.anchors["ENE"] = (
            w,
            h / 4 if cornerradius < h / 4 else h / 2 - cornerradius,
        )
        self.anchors["ESE"] = (
            w,
            -h / 4 if cornerradius < h / 4 else -h / 2 + cornerradius,
...
class Decision(Box):
    ...
    def _set_segments(self):
        ...
        self.segments.append(
            SegmentPoly([(0, 0), (w / 2, h / 2), (w, 0), (w / 2, -h / 2)])
        )
...
class Connect(Box):
    ...
    _element_defaults = {"h": 1.5, "w": 1.5, "r": 0.75, "minsize": (1.5, 1.5)}
...
class StateEnd(Connect):
    ...
    def _set_size(self) -> tuple[float, float]:
        w, h = super()._set_size()
        dr = self._userparams.get("dr", 0.15)
