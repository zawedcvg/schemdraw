...
FritzingInfo = namedtuple(
    "FritzingInfo", "author version title url label date description"
)
...
class FritzingPart(ElementImage):
    ...
    def _anchor_position(
        self, name: str, anchorelm: ET.Element
        ...
        if anchorelm.tag.endswith("circle") or anchorelm.tag.endswith("ellipse"):
            ...
            path = path.replace(
                "-", " -"
            )  # Some negative numbers don't have space before
            ...
            warnings.warn(
                f"Connector {name} unimplemented connector tag: {anchorelm.tag}"
            )
            return None, None
