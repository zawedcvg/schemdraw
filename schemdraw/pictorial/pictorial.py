...
def resistor_colors(
    value: float, tolerance: Optional[float] = None
...
class Resistor(ElementPictorial):
    ...
    _element_defaults = {"fill": "blanchedalmond"}

    def __init__(
        self, value: float = 1000, tolerance: Optional[float] = None, **kwargs
    ):
...
class Diode(ElementPictorial):
    ...
    _element_defaults = {"fill": HOUSING_COLOR, "stripe_color": "silver"}
    ...
    def __init__(self, *, stripe_color: Optional[str] = None, **kwargs):
        ...
        self.segments.append(
            Segment(((0, 0), (math.nan, math.nan), (w, 0)))
        )  # 2-term endpoints
        self.segments.append(
            SegmentPoly(
                ((0, h), (w, h), (w, -h), (0, -h)),
...
class LED(Element):
    ...
    def __init__(self, **kwargs):
        ...
        ref = PINSPACING * 5
        self.segments.append(Segment(((0, 0), (0, lead_h)), color=LEAD_COLOR, lw=4))
        self.segments.append(
            Segment(
                (
                    (PINSPACING, 0),
                    (PINSPACING, lead_h - ref * 0.3),
                    (PINSPACING * 0.6, lead_h - ref * 0.2),
                    (PINSPACING * 0.6, lead_h),
                ),
                color=LEAD_COLOR,
                lw=4,
            )
        ...
        self.segments.append(
            SegmentPath(
                (
                    ...
                    (left, lead_h),
                    "Q",
                    (center, lead_h - ref * 0.2),
                    (left + width, lead_h),
                    "L",
                    (left + width, top - top_r),
                    "C",
                    (left + width, top),
                    (left, top),
                    (left, top - top_r),
        ...
        self.segments.append(
            SegmentPath(
                (
                    "M",
                    (left, lead_h + base_h / 2),
                    "Q",
                    (center * 0.5, lead_h - ref * 0.16 + base_h),
                    (left + width * 0.7, lead_h - ref * 0.03),
                    "L",
                    (left + width + 0.0 * INCH, lead_h + base_h / 2),
                ),
                color="#FFFFFF55",
                lw=2,
            )
        ...
        reflectionx = left + 0.03 * INCH
        self.segments.append(
            SegmentPath(
                (
                    "M",
                    (reflectionx, lead_h + base_h * 2),
                    "L",
                    (reflectionx, top - top_r),
                    "Q",
                    (reflectionx, top - top_r + 0.04 * INCH),
                    (reflectionx + 0.04 * INCH, top - top_r + 0.07 * INCH),
                ),
                color="#FFFFFF55",
                lw=3,
            )
...
class LEDOrange(LED):
    ...
    _element_defaults = {"fill": "#efa207"}


class LEDYellow(LED):
    """Yellow Light Emitting Diode"""

    _element_defaults = {"fill": "#f1de0f"}


class LEDGreen(LED):
    """Green Light Emitting Diode"""

    _element_defaults = {"fill": "#4be317"}


class LEDBlue(LED):
    """Blue Light Emitting Diode"""

    _element_defaults = {"fill": "#3892bc"}


class LEDWhite(LED):
    """White Light Emitting Diode"""

    _element_defaults = {"fill": "#e5e5e5"}
...
class CapacitorElectrolytic(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.segments.append(
            SegmentArc(
                (center, top - top_r - 0.01 * INCH),
                width - 0.03 * INCH,
                top_r * 0.9,
                0,
                360,
                fill=self.params["cap_color"],
                color="none",
                lw=0.5,
                zorder=2,
            )
        ...
        self.segments.append(
            SegmentPath(
                (
                    ...
                    (left, lead_h + base_h),
                    "Q",
                    (center, lead_h - ref * 0.1),
                    (left + width, lead_h + base_h),
                ),
                color="#FFFFFF99",
                fill="none",
                lw=1.5,
        ...
        self.segments.append(
            SegmentPath(
                (
                    "M",
                    (stripex, lead_h + base_h),
                    "L",
                    (stripex, top - top_r * 2),
                ),
                color=self.params["stripe_color"],
                fill="none",
                lw=3,
            )
        )
...
class CapacitorMylar(Element):
    ...
    def __init__(self, *, lead_length: Optional[float] = None, **kwargs):
        ...
        self.segments.append(
            SegmentPath(
                (
                    ...
                    (left, lead_h),
                    "Q",
                    (left - bulge, lead_h + height / 2),
                    (left, lead_h + height),
                    "Q",
                    (center, lead_h + height + bulge),
                    (left + width, lead_h + height),
                    "Q",
                    (left + width + bulge, lead_h + height / 2),
                    (left + width, lead_h),
                    "Q",
                    (center, lead_h + bulge / 2),
                    (left, lead_h),
                    "Z",
                ),
                color="none",
        ...
        highlighty = lead_h + height - 0.025 * INCH
        self.segments.append(
            Segment(
                ((-0.01 * INCH, highlighty), (PINSPACING * 0.8, highlighty)),
                color="#FFFFFF44",
                lw=3,
            )
        )
...
class CapacitorCeramic(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        lead_h = self.params["lead_length"]
        center = PINSPACING / 2, lead_h + rad - 0.05 * INCH
        ...
        self.segments.append(SegmentCircle(center, rad, color="none", zorder=2))
...
class TO92(Element):
    ...
    def __init__(self, **kwargs):
        ...
        height = (
            0.170 * INCH * 0.9
        )  # .170 is standard height, but *.9 due to perspective
        ...
        self.segments.append(
            Segment(
                ((0, 0), (0, PINSPACING * 0.8), (PINSPACING / 2, case_y)),
                color=LEAD_COLOR,
                lw=4,
            )
        )
        self.segments.append(
            Segment(((center, 0), (center, case_y)), color=LEAD_COLOR, lw=4)
        )
        self.segments.append(
            Segment(
                (
                    (PINSPACING * 2, 0),
                    (PINSPACING * 2, PINSPACING * 0.8),
                    (3 * PINSPACING / 2, case_y),
                ),
                color=LEAD_COLOR,
                lw=4,
            )
        ...
        self.segments.append(
            SegmentPoly(
                (
                    (case_x, case_y),
                    (case_x + width, case_y),
                    (case_x + width, case_y + height),
                    (case_x, case_y + height),
                ),
                fill=HOUSING_COLOR,
                lw=1,
                color=BORDER_COLOR,
...
class DIP(Element):
    ...
    def __init__(self, npins: int = 8, wide: bool = False, **kwargs):
        ...
        pady = PINSPACING * 0.25  # padding to add above top pin and below bottom pin
        padx = PINSPACING * 0.25  # padding to remove left/right
        height = (npins // 2 - 1) * PINSPACING  # + pady*2
        center = (width) / 2
        notch_r = PINSPACING * 0.75
        body_left = padx
        ...
        for n in range(npins // 2):
            ...
            self.segments.append(
                Segment(
                    (
                        (body_right, -n * PINSPACING),
                        (body_right + padx, -n * PINSPACING),
                    ),
                    color=LEAD_COLOR,
                    lw=leadw,
                    zorder=1,
...
class Breadboard(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        strip_w = 0.37 * INCH  # Edge/power strips are 0.37 inch
        edgepad = (
            strip_w - PINSPACING
        ) / 2  # distance from edge to center of first pin in the strip
        width = 2.10 * INCH  # Total width
        height = 3.3 * INCH  # Total height
        top = (
            1.5 * edgepad
        )  # Offset the top/left so that (0, 0) is top of left power strip
        left = -edgepad
        right = left + width
        ...
        for col in range(ncols):
            ...
            self.segments.append(
                SegmentText(
                    (x + PINSPACING * 7 + col * PINSPACING, y + PINSPACING),
                    colname,
                    fontsize=8,
                    rotation_global=False,
                    align=("center", "center"),
        ...
        for row in range(0, nrows):
            self.segments.append(
                SegmentText(
                    (x - PINSPACING, y - row * PINSPACING - 0.04),
                    str(row + 1),
                    fontsize=8,
                    rotation_global=False,
                    align=("center", "bottom"),
            ...
            self.segments.append(
                SegmentText(
                    (x + PINSPACING * 12, y - row * PINSPACING - 0.04),
                    str(row + 1),
                    fontsize=8,
                    rotation_global=False,
                    align=("center", "bottom"),
        ...
        self.segments.append(
            Segment(
                (
                    (left + edgepad * 0.4, top - PINSPACING * 2),
                    (left + edgepad * 0.4, top - height + PINSPACING),
                ),
                color="red",
                lw=0.5,
            )
        ...
        self.segments.append(
            Segment(
                (
                    (left + strip_w - edgepad * 0.4, top - PINSPACING * 2),
                    (left + strip_w - edgepad * 0.4, top - height + PINSPACING),
                ),
                color="blue",
                lw=0.5,
            )
        ...
        self.segments.append(
            Segment(
                (
                    (right - edgepad * 0.4, top - PINSPACING * 2),
                    (right - edgepad * 0.4, top - height + PINSPACING),
                ),
                color="blue",
                lw=0.5,
            )
        ...
        self.segments.append(
            Segment(
                (
                    (right - strip_w + edgepad * 0.4, top - PINSPACING * 2),
                    (right - strip_w + edgepad * 0.4, top - height + PINSPACING),
                ),
                color="red",
                lw=0.5,
            )
