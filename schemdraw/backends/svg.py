...
def settextmode(mode: TextMode, svg2: bool = True) -> None:
    ...
    """
    warnings.warn(
        "settextmode is deprecated. Use schemdraw.svgconfig", DeprecationWarning
    )
    config.svg2 = svg2
    config.text = mode
...
def text_size(
    ...
    if ziamath and (mathfont is None or os.path.exists(mathfont)):
        if text == "":
            return (0, 0, 0)

        m = ziamath.Text(
            text, size=size, mathstyle=font, textfont=font, mathfont=mathfont
        )
...
class Figure:
    ...
    def __init__(self, bbox: BBox, **kwargs):
        ...
        self.scale = PT_PER_IN * kwargs.get(
            "inches_per_unit", 0.5
        )  # Converts drawing units to points
        self.margin = (
            kwargs.get("margin", 0.1) + LINE_WIDTH / PT_PER_IN
        )  # Margin in drawing units. Add line width (2pt) to include linecaps in bbox
        self.set_bbox(bbox)
    ...
    def addclip(self, et: ET.Element, bbox: Optional[BBox]):
        ...
        if bbox is not None:
            if bbox in self.clips:
                ...
                clip = ET.fromstring(
                    f'''<defs><clipPath id="clip{clipid}"><rect x="{x0 - 1}" y="{y0 - 1}"'''
    ...
    def text(
        ...
        if ziamath and config.text == "path":
            ...
            ztext.drawon(texttag, x0, y0, halign=halign, valign=valign)
        else:
            texttag = svgtext.text_tosvg(
                s,
                x0,
                y0,
                font=fontfamily,
    ...
    def arrow(
        ...
        head = Point(
            (
                head[0] - lw * 2 * math.cos(math.radians(theta)),
        ...
        et1.set(
            "style",
            getstyle(color=color, lw=0, capstyle="butt", joinstyle="miter", fill=color),
        )
    ...
    def bezier(
        ...
        if arrow is not None:
            if "<" in arrow:
                ...
                lpoints[0] = Point(
                    (
                        p[0].x - math.cos(th1) * arrowlength / 2,
            ...
            if ">" in arrow:
                ...
                lpoints[-1] = Point(
                    (
                        p[-1].x - math.cos(th2) * arrowlength / 2,
    ...
    def arc(
        ...
        startx = (
            centerx
            + width / 2 * math.cos(t2) * math.cos(anglerad)
            - height / 2 * math.sin(t2) * math.sin(anglerad)
        )
        starty = (
            centery
            + width / 2 * math.cos(t2) * math.sin(anglerad)
            + height / 2 * math.sin(t2) * math.cos(anglerad)
        )
        endx = (
            centerx
            + width / 2 * math.cos(t1) * math.cos(anglerad)
            - height / 2 * math.sin(t1) * math.sin(anglerad)
        )
        endy = (
            centery
            + width / 2 * math.cos(t1) * math.sin(anglerad)
        ...
        if arrow is not None:
            ...
            if arrow in ["ccw", "end", "both"] or ">" in arrow:
                ...
                xy = Point(
                    (
                        center[0] + width / 2 * math.cos(math.radians(th2)),
            ...
            if arrow in ["cw", "start", "both"] or "<" in arrow:
                ...
                xy = Point(
                    (
                        center[0] + width / 2 * math.cos(math.radians(th1)),
    ...
    def image(
        ...
        if imgfmt is None:
            raise ValueError(
                "Must specify image format when using file pointer with SegmentImage"
            )
    ...
    def getsvg(self) -> ET.Element:
        ...
        if self.showbbox:
            ...
            rect.set(
                "width", str(self.bbox.xmax * self.scale - self.bbox.xmin * self.scale)
            )
            rect.set(
                "height",
                str(-self.bbox.ymin * self.scale + self.bbox.ymax * self.scale),
            )
            ...
            rect.set(
                "width",
                str(
                    (self.bbox.xmax - self.margin) * self.scale
                    - (self.bbox.xmin + self.margin) * self.scale
                ),
            )
            rect.set(
                "height",
                str(
                    -(self.bbox.ymin + self.margin) * self.scale
                    + (self.bbox.ymax - self.margin) * self.scale
                ),
            )
        ...
        if not inline:
            ...
            if sys.platform == "win32":
                ...
                try:
                    ...
                    OSError,
                    FileNotFoundError,
                ):  # Some linux without display may not have xdg-open
                    pass
