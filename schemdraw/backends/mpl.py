...
class Figure:
    ...
    def __init__(self, **kwargs):
        ...
        self.inches_per_unit = kwargs.get("inches_per_unit", 0.5)
        if kwargs.get("ax"):
        ...
        self.margin = (
            kwargs.get("margin", 0.1) + 0.03
        )  # Plus half a line width for linecaps
    ...
    def addclip(self, patch, clip):
        ...
        if clip:
            cliprect = Rectangle(
                (clip.xmin, clip.ymax),
                abs(clip.xmax - clip.xmin),
                abs(clip.ymax - clip.ymin),
    ...
    def arc(
        ...
        if fill is None:
            ...
            points = [
                (
                    width / 2 * math.cos(th) + center[0],
                    height / 2 * math.sin(th) + center[1],
                )
                for th in theta
            ]
            poly = plt.Polygon(
        ...
        if not self.userfig:
            ...
            try:
                self.ax.get_figure().set_size_inches(
                    self.inches_per_unit * w, self.inches_per_unit * h
                )
            except ValueError:
