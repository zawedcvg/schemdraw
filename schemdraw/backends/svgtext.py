    ...
    for t in tokens:
        ...
        for sup in sups:
            if sup.startswith("^"):
                ...
                if chrs.isnumeric():
                    ...
                    t = t.replace(
                        sup,
                        f'<tspan baseline-shift="super" font-size="smaller">{chrs}</tspan>',
                    )
        ...
        for sup in sups:
            if sup.startswith("^"):
                t = t.replace(
                    sup,
                    f'<tspan baseline-shift="super" font-size="smaller">{sup[1]}</tspan>',
                )
        ...
        for sup in sups:
            if sup.startswith("_"):
                ...
                if chrs.isnumeric():
                    ...
                    t = t.replace(
                        sup,
                        f'<tspan baseline-shift="sub" font-size="smaller">{chrs}</tspan>',
                    )
        ...
        for sup in sups:
            if sup.startswith("_"):
                t = t.replace(
                    sup,
                    rf'<tspan baseline-shift="sub" font-size="smaller">{sup[1]}</tspan>',
                )
        ...
        for sup in sups:
            if sup.startswith(r"\sqrt{"):
                t = t.replace(sup, rf"√\overline{{{sup[6:-1]}}}")
        ...
        for sup in sups:
            if sup.startswith(r"\overline{"):
                t = t.replace(
                    sup, f'<tspan text-decoration="overline">{sup[10:-1]}</tspan>'
                )
        svgtext += t
...
def string_width(st: str, fontsize: float = 12, font: str = "Arial") -> float:
    ...
    size = 0  # in milinches
    if "times" in font.lower() or (
        "serif" in font.lower() and "sans" not in font.lower()
    ):
...
def text_approx_size(
    text: str, font: str = "Arial", size: float = 16
