...
def debug(dwgbbox: bool = True, elmbbox: bool = True):
...
class Drawing:
    ...
    def __init__(
        self,
        canvas: Optional[
            Union[Backends, xml.etree.ElementTree.Element, matplotlib.pyplot.Axes]
        ] = None,
        ...
        **kwargs,
        ...
        self.dwgparams.update(
            kwargs
        )  # To maintain support for arguments that moved to config method
    ...
    def container(
        self, cornerradius: float = 0.3, padx: float = 0.75, pady: float = 0.75
    ):
    ...
    def get_segments(self) -> list[SegmentType]:
        ...
        for element in self.elements:
            ...
            segments.extend(
                [s.xform(element.transform, **params) for s in element.segments]
            )
        return segments
    ...
    def move_from(
        self, ref: Point, dx: float = 0, dy: float = 0, theta: Optional[float] = None
    ) -> None:
    ...
    def config(
        ...
        inches_per_unit: Optional[float] = None,
    ...
    def draw(self, show: bool = True, canvas=None, backend: Optional[Backends] = None):
        """Draw the schematic
