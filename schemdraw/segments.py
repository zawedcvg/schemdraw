...
def roundcorners(verts: Sequence[XY], radius: float = 0.5) -> Sequence[XY]:
    ...
    for v in range(len(verts))[::-1]:
        ...
        arc = [
            Point(
                (
                    circlepoint[0] + math.cos(i) * radius,
                    circlepoint[1] + math.sin(i) * radius,
                )
            )
            for i in ph
        ]
...
class Segment:
    ...
    def xform(self, transform, **style) -> "Segment":
        ...
        params: dict[str, Any] = {
            ...
            if self.joinstyle
            else style.get("joinstyle", None),
            "visible": self.visible,
        }
        style = {
            k: v
            for k, v in style.items()
            if params.get(k) is None and k in params.keys()
        }
        params.update(style)
    ...
    def draw(self, fig, transform, **style) -> None:
        ...
        joinstyle = (
            self.joinstyle if self.joinstyle else style.get("joinstyle", "round")
        )
        ...
        if self.arrow and "<" in self.arrow:
            ...
            linepath[0] = Point(
                (
                    linepath[0].x + self.arrowlength * math.cos(th),
        ...
        if self.arrow and ">" in self.arrow:
            ...
            linepath[-1] = Point(
                (
                    linepath[-1].x - self.arrowlength * math.cos(th),
        ...
        if self.arrow:
            if "<" in self.arrow:
                theta = math.degrees(
                    math.atan2(path[0].y - path[1].y, path[0].x - path[1].x)
                )
                fig.arrow(
                    path[0],
                    theta,
            ...
            if ">" in self.arrow:
                theta = math.degrees(
                    math.atan2(path[-1].y - path[-2].y, path[-1].x - path[-2].x)
                )
                fig.arrow(
                    path[-1],
                    theta,
...
class SegmentText:
    ...
    def xform(self, transform, **style) -> "SegmentText":
        ...
        return SegmentText(transform.transform(self.xy), self.text, **params)
    ...
    def get_bbox(self) -> BBox:
        ...
        """
        SCALE = 2 / 72
        w, h, dy = svg.text_size(
            self.text, font=self.font, mathfont=self.mathfont, size=self.fontsize
        )
        ...
        if self.align is not None:
            ...
            if self.align[1] == "center":
                ...
                y += dy + self.fontsize * SCALE * (
                    nlines - 1
                )  # If baseline is base of BOTTOM row
    ...
    def draw(self, fig, transform, **style) -> None:
        ...
        fontsize = (
            self.fontsize
            if self.fontsize
            else style.get("fontsize", style.get("size", 14))
        )
        ...
        rotmode = (
            self.rotation_mode
            if self.rotation_mode
            else style.get("rotation_mode", "anchor")
        )
    ...
    def xform(self, transform, **style) -> "SegmentCircle" | "SegmentArc":
        ...
        if transform.zoom[0] == transform.zoom[1]:
            ...
            return SegmentCircle(
                transform.transform(self.center),
                self.radius * transform.zoom[0],
                **params,
            )
...
class SegmentArc:
    ...
    def get_bbox(self) -> BBox:
        ...
        ry = self.height / 2
        xx = [
            self.center[0] + rx * ct * cosphi - ry * st * sinphi
            for st, ct in zip(sint, cost)
        ]
        yy = [
            self.center[1] + rx * ct * sinphi + ry * st * cosphi
            for st, ct in zip(sint, cost)
        ]
...
class SegmentPath:
    ...
    def __init__(
        ...
        self.path: Sequence[XY | str] = (
            path  # Control points and strings 'M', 'L', 'C', etc.
        )
...
class SegmentImage:
    ...
    def get_bbox(self) -> BBox:
        ...
        if self.rotate % 360 == 0:
            return BBox(
                self.xy.x, self.xy.y, self.xy.x + self.width, self.xy.y + self.height
            )
