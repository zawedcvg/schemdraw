...
class ResistorVarIEEE(ResistorIEEE):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(1.5 * reswidth, -resheight * 2), (4.5 * reswidth, reswidth * 3.5)],
                arrow="->",
                arrowwidth=0.16,
                arrowlength=0.2,
            )
...
class ResistorVarIEC(ResistorIEC):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(1 * reswidth, -resheight * 2), (5 * reswidth, reswidth * 3.5)],
                arrow="->",
                arrowwidth=0.16,
                arrowlength=0.2,
            )
...
class Thermistor(ResistorIEC):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(0, -resheight - 0.2), (0.2, -resheight - 0.2), (1, resheight + 0.2)]
            )
        )
...
class PhotoresistorIEEE(ResistorIEEE):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(0.7, 0.75), (0.4, 0.4)], arrow="->", arrowwidth=0.16, arrowlength=0.2
            )
        )
        self.segments.append(
            Segment(
                [(1, 0.75), (0.7, 0.4)], arrow="->", arrowwidth=0.16, arrowlength=0.2
            )
        )
...
class Capacitor2(Element2Term):
    ...
    def __init__(self, *, polar: Optional[bool] = None, **kwargs):
        ...
        capgap = 0.18
        self.segments.append(
            Segment([(0, 0), gap, (0, resheight), (0, -resheight), gap, (capgap, 0)])
        )
        self.segments.append(
            SegmentArc(
                (capgap * 1.5, 0),
...
class CapacitorVar(Capacitor):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(-2 * reswidth, -resheight), (3 * reswidth, reswidth * 2)],
                arrow="->",
                arrowwidth=0.2,
                arrowlength=0.2,
            )
...
class CapacitorTrim(Capacitor):
    ...
    def __init__(self, **kwargs):
        ...
        t1 = Point((p2.x + tlen * math.cos(theta), p2.y + tlen * math.sin(theta)))
        t2 = Point(
            (
                p2.x + tlen * math.cos(theta - math.pi),
...
class DiodeShockley(Element2Term):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment([(0, -resheight), (0, resheight), (resheight * 1.4, 0)])
        )
        ...
        self.segments.append(
            Segment(
                [
                    ...
                    (resheight * 1.4, -resheight),
                    gap,
                    (resheight * 1.4 + capgap, resheight),
                    (resheight * 1.4 + capgap, -resheight),
                    gap,
...
class LED2(Diode):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        x = linspace(-1, 1)
        y = [-x0 * (x0 - 0.7) * (x0 + 0.7) / 2 + resheight * 2.5 for x0 in x]
        x = linspace(resheight * 0.75, resheight * 1.25)

        theta = 20
        p = [Point((x0, y0)).rotate(theta) for x0, y0 in zip(x, y)]
        p2 = [Point((x0 - 0.2, y0)).rotate(theta) for x0, y0 in zip(x, y)]

        pa = Point((x[0], y[0] + 0.1)).rotate(theta)
        pa2 = Point((x[0] - 0.2, y[0] + 0.1)).rotate(theta)

        self.segments.append(Segment(p))
        self.segments.append(Segment(p2))
        self.segments.append(
            Segment((p[1], pa), arrow="->", arrowwidth=0.15, arrowlength=0.2)
        )
        self.segments.append(
            Segment((p2[1], pa2), arrow="->", arrowwidth=0.15, arrowlength=0.2)
        )
        self.params["lblloc"] = "bot"
...
class Photodiode(Diode):
    ...
    def __init__(self, **kwargs):
        ...
        pa = Point((x[-1], y[-1] - 0.1)).rotate(theta)
        pa2 = Point((x[-1] - 0.2, y[-1] - 0.1)).rotate(theta)
        self.segments.append(Segment(p))
        self.segments.append(Segment(p2))
        self.segments.append(
            Segment((p[-2], pa), arrow="->", arrowwidth=0.15, arrowlength=0.2)
        )
        self.segments.append(
            Segment((p2[-2], pa2), arrow="->", arrowwidth=0.15, arrowlength=0.2)
        )
        self.params["lblloc"] = "bot"
...
class PotentiometerIEC(ResistorIEC):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(reswidth * 3, potheight), (reswidth * 3, reswidth * 2)],
                arrow="->",
                arrowwidth=0.15,
                arrowlength=0.22,
            )
...
class Triac(Diac):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.segments.append(
            Segment([(resheight * 1.4, 0.25), (resheight * 1.4 + 0.5, 0.5)])
        )
        self.anchors["gate"] = (resheight * 1.4 + 0.5, 0.5)
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [
                    (resheight * 1.4, 0),
                    (resheight * 1.4 + 0.3, -0.3),
                    (resheight * 1.4 + 0.3, -0.5),
                ]
            )
        )
        self.anchors["gate"] = (resheight * 1.4 + 0.3, -0.5)
...
class FuseUS(Element2Term):
    ...
    def fill(self, color: bool | str = True) -> "Element":
        """Set element fill"""
        fuser = 0.12
        self.segments.append(SegmentCircle((fuser, 0), fuser, zorder=4, fill=color))
        self.segments.append(
            SegmentCircle((fuser * 2 + 1, 0), fuser, zorder=4, fill=color)
        )
        super().fill(color)
        return self
...
class FuseIEC(ResistorIEC):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment([(6 * reswidth - dx, resheight), (6 * reswidth - dx, -resheight)])
        )
...
class Breaker(Element2Term):
    ...
    def __init__(self, *, dots: Optional[bool] = None, **kwargs):
        ...
        self.segments.append(
            SegmentArc((0.5, 0), 1, 0.65, theta1=theta1, theta2=theta2)
        )
        if self.params["dots"]:
            rad = 0.12
...
def cycloid(
    ...
    y = [
        yy * (max(y) - min(y)) / (resheight) for yy in y
    ]  # Normalize to resistor width
...
class CPE(Element2Term):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(0, 0), (-offset, resheight), gap, (0, -resheight), gap, (capgap, 0)]
            )
        )
        self.segments.append(
            Segment(
...
class SparkGap(Element2Term):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.segments.append(Segment([(0, 0), (0.3, 0), gap, (0.7, 0), (1, 0)]))
        # Arrow coords overlap a bit since default arrow is offset by linewidth
        self.segments.append(Segment([(0.3, 0), (0.52, 0)], arrow="->", arrowwidth=0.2))
        self.segments.append(Segment([(0.7, 0), (0.48, 0)], arrow="->", arrowwidth=0.2))
...
class Nullator(Element2Term):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.segments.append(
            SegmentArc(center=[0.5, 0], width=1, height=0.5, theta1=0, theta2=360)
        )
...
class VoltageMirror(Element2Term):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(Segment([[0, 0], [0, 0], gap, [1, 0], [1, 0]]))
        self.segments.append(
            SegmentArc(center=[0.7, 0], width=0.6, height=0.3, theta1=0, theta2=360)
        )
        self.segments.append(
            SegmentArc(center=[0.3, 0], width=0.6, height=0.3, theta1=0, theta2=360)
        )
        self.elmparams["theta"] = 90
