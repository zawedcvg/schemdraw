...
class Element:
    ...
    defaults: ChainMap[str, Any] = (
        ChainMap()
    )  # Subclasses will chainmap this with parents
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.anchors: MutableMapping[
            str, Union[Point, tuple[float, float]]
        ] = {}  # Untransformed anchors
        ...
        self._positioned = (
            False  # Has the element been placed in a drawing via self._position()?
        )
    ...
    def __new__(cls, *args, **kwargs):
        ...
        new.params = ChainMap(
            new._userparams, new.elmparams, new.defaults, new._dwgparams
        )
        return new
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name in anchornames + list(
            vars(self).get("anchors", {}).keys()
        ) and not name in vars(self).get("absanchors", {}):
            # Not placed yet
            drawing_stack.push_element(self)
    ...
    def up(self) -> "Element":
        ...
        if "d" in self._userparams:
            warnings.warn(
                f"Duplicated direction parameter in element. `{self._userparams['d']}` changed to `up`."
            )
    ...
    def down(self) -> "Element":
        ...
        if "d" in self._userparams:
            warnings.warn(
                "Duplicated direction parameter in element."
    ...
    def theta(self, theta: float) -> "Element":
        ...
        if "d" in self._userparams:
            warnings.warn(
                "Duplicate direciton parameter in element:"
    ...
    def at(
        self, xy: XY | tuple["Element", str], dx: float = 0, dy: float = 0
    ) -> "Element":
        ...
        if "at" in self._userparams:
            warnings.warn(
                "Duplicate `at` parameter in element: "
    ...
    def anchor(self, anchor: str) -> "Element":
        ...
        if "anchor" in self._userparams:
            warnings.warn(
                "Duplicate anchor parameter in element: "
    ...
    def _place(self, dwgxy: XY, dwgtheta: float, **dwgparams) -> tuple[Point, float]:
        ...
        if drop is not None and drop in self.anchors:
            ...
            if self.params.get("droptheta", None) is not None:
                ...
                self.absdrop = (
                    self.transform.transform(drop),
                    self.params.get("droptheta", 0.0),
                )
    ...
    def _align_label(
        self, label: Label, theta: float = 0
    ) -> tuple[Halign, Valign, Point]:
        ...
        if label.loc == "center":
            ...
            if math.isclose(self.anchors[label.loc][0], x1, abs_tol=0.15):
                # Label on left edge
                newhalign = "right"
            elif math.isclose(self.anchors[label.loc][0], x2, abs_tol=0.15):
                # Label on right edge
                newhalign = "left"
            else:
            ...
            if math.isclose(self.anchors[label.loc][1], y1, abs_tol=0.15):
                # Label on bottom edge
                newvalign = "top"
            elif math.isclose(self.anchors[label.loc][1], y2, abs_tol=0.15):
                # Label on top edge
                newvalign = "bottom"
            else:
            ...
            th = {"left": th + 90, "bottom": th + 180, "right": th + 270}.get(
                label.loc, th
            )  # type: ignore
    ...
    def _place_label(self, label: Label, theta: float = 0) -> None:
        ...
        if label.ofst is None:
            label.ofst = self.params.get("lblofst", 0.1)
        ...
        segment_params = {
            ...
            "mathfont": label.mathfont
            if label.mathfont
            else self.params.get("mathfont"),
            "fontsize": label.fontsize
            if label.fontsize
            else self.params.get("fontsize", 14),
        ...
        if isinstance(label.label, (list, tuple)):
            if label.loc == "top":
                ...
                for i, lbltxt in enumerate(label.label):
                    ...
                    self.segments.append(
                        SegmentText(xy + label.ofst, lbltxt, **segment_params)
                    )
            elif label.loc == "bottom":
                xdiv = (xmax - xmin) / (len(label.label) + 1)
                for i, lbltxt in enumerate(label.label):
            ...
            label.label, str
        ):  # keep the elif instead of else for type hinting
            ...
            self.segments.append(
                SegmentText(xy + label.ofst, label.label, **segment_params)
            )
...
class Element2Term(Element):
    ...
    def down(self, length: Optional[float] = None) -> "Element":
        ...
        if "d" in self._userparams:
            warnings.warn(
                f"Duplicated direction parameter in element. `{self._userparams['d']}` changed to `down`."
            )
        self._userparams["d"] = "down"
        if length:
            self._userparams["l"] = length
    ...
    def left(self, length: Optional[float] = None) -> "Element":
        ...
        if "d" in self._userparams:
            warnings.warn(
                f"Duplicated direction parameter in element. `{self._userparams['d']}` changed to `left`."
            )
        self._userparams["d"] = "left"
        if length:
            self._userparams["l"] = length
    ...
    def right(self, length: Optional[float] = None) -> "Element":
        ...
        if "d" in self._userparams:
            warnings.warn(
                f"Duplicated direction parameter in element. `{self._userparams['d']}` changed to `right`."
            )
        self._userparams["d"] = "right"
        if length:
            self._userparams["l"] = length
    ...
    def _place(self, dwgxy: XY, dwgtheta: float, **dwgparams) -> tuple[Point, float]:
        ...
        if self.params.get("idot", False):
            ...
            radius = self.params.get("dotradius", 0.075)
            self.segments.append(
                SegmentCircle(start, radius=radius, fill=fill, zorder=3)
            )
