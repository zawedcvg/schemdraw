...
class Line(Element2Term):
    ...
    _element_defaults = {"arrowwidth": 0.15, "arrowlength": 0.25}
    ...
    def __init__(self, *, arrow: Optional[str] = None, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(0, 0)], arrow=arrow, arrowwidth=arrowwidth, arrowlength=arrowlength
            )
        )
...
class Dot(Element):
    ...
    _element_defaults = {"radius": 0.075, "open": False}

    def __init__(
        self, *, radius: Optional[float] = None, open: Optional[bool] = None, **kwargs
    ):
        super().__init__(**kwargs)
...
class Arrowhead(Element):
    ...
    _element_defaults = {"headwidth": 0.15, "headlength": 0.25, "lblofst": 0.25}
...
class Wire(Element):
    ...
    def __init__(
        self, shape: str = "-", k: float = 1, *, arrow: Optional[str] = None, **kwargs
    ):
        super().__init__(**kwargs)
    ...
    def _place(self, dwgxy: XY, dwgtheta: float, **dwgparams) -> tuple[Point, float]:
        ...
        if shape == "-":  # Straight line
            ...
            if dx > 0:
                ...
                k = -abs(k)
            self.segments.append(
                Segment([(0, 0), (k, 0), (dx - k, dy), (dx, dy)], arrow=arrow)
            )
            self.anchors["mid"] = (dx / 2, dy / 2)
            self.elmparams["droptheta"] = 0 if dx > 0 else 180
        elif shape == "N":
...
class Arc2(Element):
    ...
    def _place(self, dwgxy: XY, dwgtheta: float, **dwgparams) -> tuple[Point, float]:
        ...
        self.segments.append(
            SegmentBezier((Point((0, 0)), pa, Point((dx, dy))), arrow=self.arrow)
        )
        ...
        halign = "left"
        vofst = 0.1
        if math.isclose(mid.y, pa.y, abs_tol=0.01):
            valign = "center"
            vofst = 0
        elif mid.y > pa.y:
            valign = "top"
            vofst = -0.1
        if math.isclose(mid.x, pa.x, abs_tol=0.01):
            halign = "center"
        elif mid.x > pa.x:
            halign = "right"
...
class Arc3(Element):
    ...
    _element_defaults = {"arrowlength": 0.25, "arrowwidth": 0.2}
    ...
    def _place(self, dwgxy: XY, dwgtheta: float, **dwgparams) -> tuple[Point, float]:
        ...
        if delta is not None:
            ...
            dy = to.y - xy.y
        pa1 = Point(
            (dx * self.k * math.cos(self.th1), dy * self.k * math.sin(self.th1))
        )
        pa2 = Point(
            (
                dx + dx * self.k * math.cos(self.th2),
                dy + dy * self.k * math.sin(self.th2),
            )
        )
        self.segments.append(
            SegmentBezier(
                (Point((0, 0)), pa1, pa2, Point((dx, dy))),
                arrow=self.arrow,
...
class Annotate(Arc3):
    ...
    def _setlabel(self, dx: float, dy: float) -> None:
        ...
        if th2 < 45 or th2 > 315:
            ...
            self.elmparams["lblofst"] = (0.1, 0)
        elif 45 <= th2 <= 135:
            self.elmparams["lblalign"] = ("center", "bottom")
            self.elmparams["lblofst"] = (0, 0.1)
        elif 135 < th2 <= 225:
...
class ArcLoop(Element):
    ...
    def _place(self, dwgxy: XY, dwgtheta: float, **dwgparams) -> tuple[Point, float]:
        ...
        try:
            center = Point(
                (
                    (xa + math.sqrt(self.radius**2 - (q / 2) ** 2) * (-dy / q)),
                    (ya + math.sqrt(self.radius**2 - (q / 2) ** 2) * (dx / q)),
                )
            )
        except ValueError as err:
            raise ValueError(
                f"No solution to ArcLoop with radius {self.radius}"
            ) from err
...
class Tag(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        h = h / 2
        self.segments.append(SegmentPoly([(0, 0), (h, h), (w, h), (w, -h), (h, -h)]))
        self.anchors["start"] = (0, 0)
...
class CurrentLabel(Element):
    ...
    def _place(self, dwgxy, dwgtheta, **dwgparams):
        ...
        if self._side in ["right"] and (theta <= 90 or theta > 270):
            ...
            self.elmparams["lblloc"] = {"bot": "top", "top": "bot"}.get(loc)
        ...
        headlength = self.params["headlength"]
        self.segments.append(
            Segment((a, b), arrow="->", arrowwidth=headwidth, arrowlength=headlength)
        )
        return super()._place(dwgxy, dwgtheta, **dwgparams)
...
class CurrentLabelInline(Element):
    ...
    _element_defaults = {"headlength": 0.3, "headwidth": 0.3}
...
class ZLabel(Element):
    ...
    def _place(self, dwgxy, dwgtheta, **dwgparams):
        ...
        b = Point(
            (-self.params["hofst"] - self.params["lengthtip"], self.params["ofst"])
        )
        c = Point(
            (
                -self.params["hofst"] - self.params["lengthtip"],
...
class LoopCurrent(LoopArrow):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        drawing_stack.push_element(
            None
        )  # Flush the stack to make sure all elm_list are placed
...
class Encircle(Element):
    ...
    _element_defaults = {"padx": 0.2, "pady": 0.2, "theta": 0}
...
class EncircleBox(Element):
    ...
    _element_defaults = {"cornerradius": 0.3, "padx": 0.2, "pady": 0.2, "theta": 0}
