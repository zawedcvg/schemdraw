...
class OutletA(Element):
    ...
    def __init__(self, plug: bool = False, **kwargs):
        ...
        xleft = [0.15 + self._outletrad * math.cos(t) for t in th]
        yleft = [self._outletrad * math.sin(t) for t in th]
        xrght = [-0.15 + -self._outletrad * math.cos(t) for t in th]
...
class OutletB(OutletA):
    ...
    def __init__(self, plug: bool = False, **kwargs):
        ...
        self.anchors["neutral"] = (
            self.anchors["neutral"][0],
            self.anchors["neutral"][1] + dy,
        )
...
class OutletI(OutletH):
    ...
    def __init__(self, plug: bool = False, **kwargs):
        ...
        hotcenter = (0.4, 0.3)
        neutcenter = (-0.4, 0.3)
        self.segments[2].verts = [
            rotate(v, 90, center=hotcenter) for v in self.segments[2].verts
        ]  # type: ignore
        self.segments[3].verts = [
            rotate(v, -90, center=neutcenter) for v in self.segments[3].verts
        ]  # type: ignore
        self.anchors["hot"] = rotate(self.anchors["hot"], 90, center=hotcenter)
        self.anchors["neutral"] = rotate(
            self.anchors["neutral"], -90, center=neutcenter
        )
