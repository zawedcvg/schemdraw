...
class Ic(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._dflt_side = IcSide(
            ...
            self.params.get("edgepadH", 0.25),
            self.params.get("leadlen", 0.5),
            self.params.get("lofst", 0.15),
            self.params.get("lsize", 14),
            self.params.get("plblofst", 0.05),
    ...
    def side(
        ...
        """
        side = cast(Side, side[0].upper())
        self.usersides[side] = IcSide(
            spacing, pad, leadlen, label_ofst, label_size, pinlabel_ofst, pinlabel_size
        )
        self._setsize()
        return self
    ...
    def _autosize(self) -> None:
        ...
        for side in ["L", "R", "T", "B"]:
            ...
            lengths[side] = sideparam.pad * 2 + sideparam.spacing * (
                self.pincount[side] - 1
            )
        ...
        boxh = max(
            lengths.get("L", 0.0), lengths.get("R", 0.0), 2 + self.params["edgepadH"]
        )
        boxw = max(
            lengths.get("T", 0.0),
            lengths.get("B", 0.0),
            2 + self.params["edgepadW"],
            labelw,
        )
    ...
    def _autopinlayout(self) -> None:
        ...
        for side in ["L", "R", "T", "B"]:
            ...
            if sideparam.spacing == 0:
                ...
                sideparam.pad = (
                    length - sideparam.spacing * (self.pincount[side] - 1)
                ) / 2
            ...
            xy = Point(
                (
                    (
                        xy[0],
                        xy[1]
                        + (self._icbox.y2 - self._icbox.h)
                        - xy[0] * math.tan(-math.radians(self.slant)),
                    )
                )
            )
    ...
    def _drawpin(self, side: Side, pin: IcPin, num: int) -> None:
        ...
        leadext = {
            "L": Point((-sidesetup.leadlen, 0)),
            "R": Point((sidesetup.leadlen, 0)),
            "T": Point((0, sidesetup.leadlen)),
            "B": Point((0, -sidesetup.leadlen)),
        }.get(side, Point((0, 0)))
        ...
        if sidesetup.leadlen > 0:
            if pin.invert:  # Add invert-bubble
                ...
                invertofst = {
                    "L": Point((-invertradius, 0)),
                    "R": Point((invertradius, 0)),
                    "T": Point((0, invertradius)),
                    "B": Point((0, -invertradius)),
                }.get(side, Point((0, 0)))
                self.segments.append(SegmentCircle(xy + invertofst, invertradius))
        ...
        if pin.pin and pin.pin != "":
            ...
            pofst = {
                "L": Point((-plbl - invertradius * 2, plbl)),
            ...
            self.segments.append(
                SegmentText(
                    ...
                    label=pin.pin,
                    align=align,
                    fontsize=pin.pinlblsize
                    if pin.pinlblsize is not None
        ...
        if pin.name and pin.name != "":
            pofst = {
                "L": Point((sidesetup.label_ofst, 0)),
                "R": Point((-sidesetup.label_ofst, 0)),
                "T": Point((0, -sidesetup.label_ofst)),
                "B": Point((0, sidesetup.label_ofst)),
            }.get(side)
            ...
            self.segments.append(
                SegmentText(
                    ...
                    label=pin.name,
                    align=align,
                    fontsize=pin.lblsize
                    if pin.lblsize is not None
    ...
    def _drawclkpin(
        self, xy: Point, leadext: Point, side: Side, pin: IcPin, num: int
    ) -> None:
        """Draw clock pin >"""
        sidesetup = self.sides.get(side, self._dflt_side)
...
class Multiplexer(Ic):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(size=size, pins=pins, slant=slant, **kwargs)
        ...
        **kwargs,
        ...
        for i in range(self.params["pins"] // 2):
            ...
            if self.params["number"]:
                self.segments.append(
                    SegmentText(
                        (-pw / 2, ymid), str(pnum), fontsize=self.params["pfontsize"]
                    )
                )
            if names:
                self.segments.append(
            ...
            # Right
            self.segments.append(
                SegmentPoly(((w, y1), (w, y2), (w + pw, y2), (w + pw, y1)))
            )
            ...
            if self.params["number"]:
                self.segments.append(
                    SegmentText(
                        (w + pw / 2, ymid), str(pnum), fontsize=self.params["pfontsize"]
                    )
                )
            if names:
                self.segments.append(
...
class DFlipFlop(Ic):
    ...
    def __init__(self, preclr: bool = False, preclrinvert: bool = True, **kwargs):
        ...
        if preclr:
            pins.extend(
                [
                    IcPin("PRE", side="top", invert=preclrinvert),
...
class VoltageRegulator(Ic):
    ...
    _element_defaults = {"size": (2, 1.5)}

    def __init__(self, **kwargs):
        pins = [
            IcPin("in", side="left", slot="3/3"),
...
class Ic555(Ic):
    ...
    def __init__(self, **kwargs):
        pins = [
            IcPin(name="TRG", side="left", pin="2"),
...
def sevensegdigit(
    ...
    segClabel = (seglen + halfw + halfspace, seglen * 0.5)
    segElabel = (-halfw - halfspace, seglen * 0.5)
    ...
    fillA = (
        segcolor
        if str(digit).lower()
        in ["2", "3", "5", "6", "7", "8", "9", "0", "a", "c", "e", "f"]
        else None
    )
    fillB = (
        segcolor
        if str(digit).lower() in ["1", "2", "3", "4", "7", "8", "9", "0", "a", "d"]
        else None
    )
    fillC = (
        segcolor
        if str(digit).lower()
        in ["1", "3", "4", "5", "6", "7", "8", "9", "0", "a", "b", "d"]
        else None
    )
    fillD = (
        segcolor
        if str(digit).lower() in ["2", "3", "5", "6", "8", "9", "0", "b", "c", "d", "e"]
        else None
    )
    fillE = (
        segcolor
        if str(digit).lower() in ["2", "6", "8", "0", "a", "b", "c", "d", "e", "f"]
        else None
    )
    fillF = (
        segcolor
        if str(digit).lower() in ["4", "5", "6", "8", "9", "0", "a", "b", "c", "e", "f"]
        else None
    )
    fillG = (
        segcolor
        if str(digit).lower()
        in ["2", "3", "4", "5", "6", "8", "9", "a", "b", "d", "e", "f"]
        else None
    )

    segments: list[SegmentType] = []
    segments.append(
        SegmentPoly(list(zip(segAx, segAy)), color="gray", fill=fillA, lw=0.5)
    )
    segments.append(
        SegmentPoly(list(zip(segBx, segBy)), color="gray", fill=fillB, lw=0.5)
    )
    segments.append(
        SegmentPoly(list(zip(segCx, segCy)), color="gray", fill=fillC, lw=0.5)
    )
    segments.append(
        SegmentPoly(list(zip(segDx, segDy)), color="gray", fill=fillD, lw=0.5)
    )
    segments.append(
        SegmentPoly(list(zip(segEx, segEy)), color="gray", fill=fillE, lw=0.5)
    )
    segments.append(
        SegmentPoly(list(zip(segFx, segFy)), color="gray", fill=fillF, lw=0.5)
    )
    segments.append(
        SegmentPoly(list(zip(segGx, segGy)), color="gray", fill=fillG, lw=0.5)
    )
    if labelsegments:
        segments.append(
            SegmentText(segAlabel, "a", align=("center", "bottom"), fontsize=10)
        )
        segments.append(
            SegmentText(segGlabel, "g", align=("center", "bottom"), fontsize=10)
        )
        segments.append(
            SegmentText(segDlabel, "d", align=("center", "bottom"), fontsize=10)
        )
        segments.append(
            SegmentText(segBlabel, "b", align=("left", "center"), fontsize=10)
        )
        segments.append(
            SegmentText(segClabel, "c", align=("left", "center"), fontsize=10)
        )
        segments.append(
            SegmentText(segElabel, "e", align=("right", "center"), fontsize=10)
        )
        segments.append(
            SegmentText(segFlabel, "f", align=("right", "center"), fontsize=10)
        )
    ...
    if decimal:
        ...
        segments.append(
            SegmentCircle(
                (left + seglen + segw + dotrad / 2, bottom),
                radius=dotrad,
                color="gray",
                fill=segcolor,
                lw=0.5,
...
class SevenSegment(Ic):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        pins = [
            IcPin(pin="a", side="left", slot=f"{7 + dec}/{slots}", anchorname="a"),
        ...
        if dec:
            pins.append(
                IcPin(pin="dp", side="left", slot=f"1/{slots}", anchorname="dp")
            )
