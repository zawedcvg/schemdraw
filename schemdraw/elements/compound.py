...
class ElementCompound(elm.Element):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.dwgparams = {
            "unit": kwargs.get("unit", 3),
            "font": kwargs.get("font", None),
            "fontsize": kwargs.get("fontsize", 14),
            "lblofst": kwargs.get("lblofst", 0.1),
    ...
    def move_from(
        self, xy: Point, dx: float = 0, dy: float = 0, theta: Optional[float] = None
    ) -> None:
    ...
    def add(self, element: elm.Element) -> elm.Element:
        ...
        self._here, self._theta = element._place(
            self._here, self._theta, **self.dwgparams
        )
        self.segments.extend(
            [s.xform(element.transform, **element.params) for s in element.segments]
        )
        return element
    ...
    def setup(self):
        ...
        if self.base:
            self.add(
                elm.Line(
                    d="l",
                    at=Point((B.get_bbox(transform=True).xmin, D.center[1])),
                    l=0.15,
                )
            )  # type: ignore
...
class Relay(ElementCompound):
    ...
    def setup(self) -> None:
        ...
        if self.core:
            self.add(
                elm.Line(d="d", at=(Lleft + 0.15, -self.unit / 2), anchor="center", l=1)
            )
            swleft += 0.1
        ...
        if self.switch == "spst":
            ...
            bbox = SW.get_bbox()
            SW._userparams["at"] = (
                swleft - (-bbox.ymax if self.swflip else bbox.ymin),
                0,
            )
            self.add(SW)
            ...
            SW = elm.SwitchDpst(
                d="d", link=False, reverse=self.swreverse, flip=self.swflip
            )
            bbox = SW.get_bbox()
            SW._userparams["at"] = (
                swleft - (-bbox.ymax if self.swflip else bbox.ymin),
        ...
        if self.link:
            ...
            self.add(elm.Line(d="r", at=(Lleft + 0.2, -self.unit / 2), ls=":", tox=tox))
        ...
        if self.box:
            ...
            self.add(
                elm.Rect(
                    d="r",
                    at=(0, 0),
                    corner1=(bbox.xmin - self.boxpad, bbox.ymin + 0.2),
                    corner2=(bbox.xmax + self.boxpad, bbox.ymax - 0.2),
                    fill=self.boxfill,
                    zorder=0,
                )
            )
...
class Wheatstone(ElementCompound):
    ...
    def __init__(
        self, vout: bool = False, labels: Optional[Sequence[str]] = None, **kwargs
    ):
        self.vout = vout
        self.labels = labels
        super().__init__(**kwargs)
