...
class Opamp(Element):
    ...
    _element_defaults = {"sign": True, "leads": False}

    def __init__(
        self, *, sign: Optional[bool] = None, leads: Optional[bool] = None, **kwargs
    ):
        ...
        if self.params["leads"]:
            ...
            self.segments.append(Segment([(0, -oa_back / 4), (leadlen, -oa_back / 4)]))
            self.segments.append(
                Segment([(leadlen + oa_xlen, 0), (oa_xlen + 2 * leadlen, 0)])
            )
        ...
        self.anchors["vd"] = (x + oa_xlen / 3, 0.84)
        self.anchors["vs"] = (x + oa_xlen / 3, -0.84)
        self.anchors["n1"] = (x + oa_xlen * 2 / 3, -0.42)
        self.anchors["n2"] = (x + oa_xlen * 2 / 3, 0.42)
        self.anchors["n1a"] = (x + oa_xlen * 0.9, -0.13)
        self.anchors["n2a"] = (x + oa_xlen * 0.9, 0.13)
