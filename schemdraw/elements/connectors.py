        ...
        if abs(dy) < 0.05:
            ...
            for i in range(n):
                ...
                self.segments.append(
                    Segment([(0, y), (x, y), (x, y + dy), (dx, y + dy)], arrow=arrow)
                )
...
class RightLines(Element):
    ...
    _element_defaults = {"dy": 0.6, "to": (3, -2), "theta": 0, "n": 1}
        ...
        **kwargs,
        ...
        for row in range(rows):
            for col in range(cols):
                xy = (
                    col * self.params["pinspacing"] + self.params["edge"],
                ...
                if self.params["style"] == "square":
                    ...
                    self.segments.append(
                        SegmentCircle(
                            xy, pinrad * 1.75, fill=self.params["pinfill"], zorder=4
                        )
                    )
                    self.segments.append(
                        Segment(
                    ...
                    self.segments.append(
                        SegmentCircle(xy, pinrad, fill=self.params["pinfill"], zorder=4)
                    )
                ...
                if self.params["numbering"] == "lr" or self.params["numbering"] is True:
                    ...
                    pnumber = (
                        str(rows * col + (rows - row)) if col % 2 else str(row + 1)
                    )
                self.anchors["pin{}".format(pnumber)] = xy

                if self.params["shownumber"]:
                    numxy = (w + 0.05 if col % 2 else -0.05, xy[1])
                    align = ("left" if col % 2 else "right", "bottom")
                    self.segments.append(
                        SegmentText(
                            numxy,
                            pnumber,
                            fontsize=self.params["pinfontsizeleft"],
                            align=align,
                        )
                    )  # type: ignore
                ...
                if pinsright and (cols == 1 or col % 2):
                    ...
                    self.segments.append(
                        SegmentText(
                            lblxy,
                            pinsright[row],
                            fontsize=self.params["pinfontsizeright"],
...
class Jumper(Element):
    ...
    _element_defaults = {"pinspacing": 0.6, "pinrad": 0.1, "theta": 0}

    def __init__(self, *, pinspacing: Optional[float] = None, **kwargs):
        super().__init__(**kwargs)
        pinrad = self.params["pinrad"]
        x = pinrad * 2.5
        self.segments.append(
...
class BusConnect(Element):
    ...
    _element_defaults = {"dy": 0.6, "lwbus": 4, "l": 3, "slantx": 0.5}
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(n):
            y = -i * deltay
            self.segments.append(
                Segment([(0, y), (deltax - slantx, y), (deltax, y + slanty)])
            )
            self.anchors[f"pin{i + 1}"] = (0, y)
        self.segments.append(
            Segment([(deltax, slantx), (deltax, slanty - n * deltay)], lw=lw)
        )
...
class DB9(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.segments.append(
            SegmentPoly([(0, 0), (0, h1), (w, h2), (w, -0.5)], cornerradius=0.25)
        )

        for i in range(4):
            xy = (edgepad, h1 - (i + 0.5) * spacing - edgepad)
        ...
        for i in range(5):
            xy = (edgepad + spacing, h2 - (i + 0.75) * spacing - edgepad)
            self.segments.append(SegmentCircle(xy, pinrad, fill=fill, zorder=4))
...
class CoaxConnect(Element):
    ...
    _element_defaults = {"radius": 0.4, "radiusinner": 0.12, "fillinner": "bg"}
...
class Plug(Element):
    ...
    _element_defaults = {"pluggap": 0.18}
...
class Terminal(Element):
    ...
    def __init__(
        self, *, r: Optional[float] = None, open: Optional[bool] = None, **kwargs
    ):
        super().__init__(**kwargs)
        ...
        ky = 2.0
        self.segments.append(
            Segment([(-kx * radius, -ky * radius), (kx * radius, ky * radius)])
        )
