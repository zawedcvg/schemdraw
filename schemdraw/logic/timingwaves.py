...
def getsplit(x0: float, y0: float, y1: float, **kwargs) -> list:
    ...
    segments = [
        Segment(left, lw=1, zorder=3),
        Segment(right, lw=1, zorder=3),
        SegmentPoly(
            left + right[::-1], zorder=3, color="none", fill="bg", lw=1, closed=False
        ),
    ]
    return segments
...
class Doublesigmoid:
    ...
    def curve(
        self, side: str = "left", crop: bool = False, ofst: float = 0
...
class Wave0:
    ...
    def __init__(self, params):
        ...
        self.y1 = self.params.get("y1", 0.5)
...
class WaveV(Wave0):
    ...
    def verts_out(self) -> list[tuple[float, float]]:
        ...
        verts = {
            ...
            "h": [(self.xend, self.y0), (self.xend, self.y1)],
            "z": (
                list(zip(xcurve, [yc - (self.y1 - self.y0) / 2 for yc in ycurvehf]))
                + list(
                    zip(
                        xcurve[::-1],
                        [yc + (self.y1 - self.y0) / 2 for yc in ycurveh[::-1]],
                    )
                )
            ),
            "V": [
                (self.xend, self.y0),
                (self.xend + self.rise / 2, self.yhalf),
                (self.xend, self.y1),
            ],
    ...
    def fillcolor(self):
        ...
        fill = {
            ...
            "8": "#e3a5fa",
            "9": "#f7b7bd",
        }.get(self.params.get("state", "2"), None)
    ...
    def segments(self) -> list[SegmentType]:
        ...
        if (
            self.nstate in "-|" and self.pstate in "-|"
        ):  # Open both ends. Draw two lines and a poly
            ukwargs["color"] = "none"
            segments.append(
                Segment([(self.x0, self.y0), (self.xend, self.y0)], **self.kwargs)
            )
            segments.append(
                Segment([(self.x0, self.y1), (self.xend, self.y1)], **self.kwargs)
            )
            segments.append(
                SegmentPoly(
                    [
            ...
            segments.append(
                SegmentPoly(
                    [(self.xend, self.y1)] + self.verts_in() + [(self.xend, self.y0)],
                    closed=False,
                    **ukwargs,
                )
            )
