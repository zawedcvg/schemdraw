    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.segments.append(
            Segment([(0, 0), (w, 0), (w, h), (0, h), (0, 0)], **kwargs)
        )
        ...
        self.segments.append(
            SegmentText(
                (0, h + 3 * diag / 4), topnames, align=("right", "bottom"), fontsize=12
            )
        )
        self.segments.append(
            SegmentText(
                (-3 * diag / 4, h), leftnames, align=("right", "bottom"), fontsize=12
            )
        )
        for i, col in enumerate(order[:cols]):
            self.segments.append(
                SegmentText(
                    (col * boxw + boxw / 2, h + boxw / 20),
                    format(i, colfmt),
                    align=("center", "bottom"),
                    fontsize=10,
                )
        ...
        for i, col in enumerate(order[:cols]):
            for j, row in enumerate(order[:rows]):
                invalue = "".join(
                    [k for k in format(i, colfmt)] + [k for k in format(j, rowfmt)]
                )
                try:
                ...
                self.segments.append(
                    SegmentText(
                        (col * boxw + boxw / 2, h - row * boxw - boxw / 2),
                        valstr,
                        align=("center", "center"),
                        fontsize=12,
                    )
                )
                self.anchors["cell" + "".join(str(k) for k in invalue)] = (
                    col * boxw + boxw / 2,
                    h - row * boxw - boxw / 2,
                )
        ...
        for group, style in groups.items():
            ...
            llen = len(leftnames)
            gwidth = int(
                2**tlen / (2 ** (group[:tlen].count("0") + group[:tlen].count("1")))
            )
            gheight = int(
                2**llen / (2 ** (group[tlen:].count("0") + group[tlen:].count("1")))
            )
            rowlookup = {
                "0": 0,
                "1": 1,
            ...
            if col + gwidth <= cols and row + gheight <= rows:
                ...
                verts = [
                    [
                        (x1 + gpad, y1 - gpad),
                        (x1 + boxw + gpad, y1 - gpad),
                        (x1 + boxw + gpad, y2 + gpad),
                        (x1 + gpad, y2 + gpad),
                    ],
                    [
                        (-gpad, y1 - gpad),
                ...
                verts = [
                    [
                        (x1 + gpad, y1 - gpad),
                        (x2 - gpad, y1 - gpad),
                        (x2 - gpad, y1 - boxw - gpad),
                        (x1 + gpad, y1 - boxw - gpad),
                    ],
                    [
                        (x1 + gpad, h + gpad),
                        (x2 - gpad, h + gpad),
                ...
                verts = [
                    [
                        (x1 + gpad, y1 - gpad),
                        (x1 + boxw + gpad, y1 - gpad),
                        (x1 + boxw + gpad, y1 - boxw - gpad),
                        (x1 + gpad, y1 - boxw - gpad),
                    ],
                    [
                        (-gpad, y1 - gpad),
                        (boxw - gpad, y1 - gpad),
                        (boxw - gpad, y1 - boxw - gpad),
                        (-gpad, y1 - boxw - gpad),
                    ],
                    [
                        (-gpad, h + gpad),
                        (boxw - gpad, h + gpad),
                        (boxw - gpad, h - boxw + gpad),
                        (-gpad, h - boxw + gpad),
                    ],
                    [
                        (x1 + gpad, h + gpad),
                        (x1 + boxw + gpad, h + gpad),
                        (x1 + boxw + gpad, h - boxw + gpad),
                        (x1 + gpad, h - boxw + gpad),
                    ],
                ]
            ...
            for vert in verts:
                self.segments.append(SegmentPoly(vert, cornerradius=0.25, **style))
