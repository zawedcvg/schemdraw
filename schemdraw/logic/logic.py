...
class And(Element):
    ...
    _element_defaults = {"leadin": 0.35, "leadout": 0.35}

    def __init__(
        self,
        inputs: int = 2,
        nand: bool = False,
        inputnots: Optional[Sequence[int]] = None,
        *,
        ...
        **kwargs,
        ...
        if nand:
            self.segments.append(
                SegmentCircle((_leadin + gatel + gateh / 2 + notbubble, 0), notbubble)
            )
        ...
        if inputs > 3:
            self.segments.append(
                Segment(
                    [(_leadin, backlen / 2 + dy / 2), (_leadin, -backlen / 2 - dy / 2)]
                )
            )
        ...
        if nand:
            self.segments.append(
                Segment(
                    [
                        (gatel + gateh / 2 + _leadin + notbubble * 2, 0),
                        (gatel + gateh / 2 + _leadin + _leadout, 0),
                    ]
                )
            )
        else:
...
class Or(Element):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if inputs > 3:
            self.segments.append(
                Segment([(_leadin, backlen / 2 + dy / 2), (_leadin, orheight)])
            )
            self.segments.append(
                Segment([(_leadin, -backlen / 2 - dy / 2), (_leadin, -orheight)])
            )
        ...
        if nor:
            self.segments.append(
                Segment([(tip + notbubble * 2, 0), (tip + _leadout, 0)])
            )
        else:
...
class Buf(Element2Term):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment([(0, gateh / 2), (gatel, 0), (0, -gateh / 2), (0, gateh / 2)])
        )
...
class Tristate(Element2Term):
    ...
    def __init__(self, outputnot: bool = True, controlnot: bool = False, **kwargs):
        ...
        if controlnot:
            self.segments.append(SegmentCircle((gatel / 2, 0.42), notbubble))
            self.segments.append(
                Segment([(gatel / 2, 0.42 + notbubble), (gatel / 2, 0.7)])
            )
        else:
            self.segments.append(Segment([(gatel / 2, 0.28), (gatel / 2, 0.7)]))
        self.anchors["out"] = (gatel + notbubble * 2, 0)
        self.anchors["in1"] = (0, 0)
        self.anchors["c"] = (gatel / 2, 0.7)
...
class NotNot(Element2Term):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.segments.append(
            Segment([(-notbubble * 2, 0), gap, (gatel + notbubble * 2, 0)])
        )
        self.segments.append(
            Segment([(0, gateh / 2), (gatel, 0), (0, -gateh / 2), (0, gateh / 2)])
        )
...
class Tgate(Element2Term):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(Segment([(0, 0), gap, (gatel, 0)]))
        self.segments.append(
            Segment([(0, 0), (0, -gateh / 2), (gatel, 0), (0, gateh / 2), (0, 0)])
        )
        self.segments.append(
            Segment(
                [
        ...
        self.segments.append(SegmentCircle((gatel / 2, 0.28 + 0.08), 0.08))
        self.segments.append(Segment([(gatel / 2, 0.28 + 0.16), (gatel / 2, 0.7)]))
        self.segments.append(Segment([(gatel / 2, -0.28), (gatel / 2, -0.7)]))
        self.anchors["out"] = (gatel, 0)
        self.anchors["in1"] = (0, 0)
        self.anchors["c"] = (gatel / 2, -0.7)
        self.anchors["cbar"] = (gatel / 2, 0.7)
