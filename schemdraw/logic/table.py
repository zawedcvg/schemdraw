    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(rows[0].strip("| ").split("|")) != ncols:
            raise ValueError(
                "Number of columns in table does not match number of columns in colfmt string."
            )

        colwidths = [0.0] * ncols
        rowheights = [0.0] * nrows
        ...
        for i, colbar in enumerate(colbars):
            ...
            if colbar == "|":
                ...
                self.segments.append(
                    Segment(
                        [(colx - doublebarsep, 0), (colx - doublebarsep, -totheight)],
                        **kwargs,
                    )
                )
                self.segments.append(
        ...
        for i, rowbar in enumerate(rowbars):
            ...
            if rowbar == "|":
                ...
                self.segments.append(
                    Segment(
                        [(0, rowy - doublebarsep), (totwidth, rowy - doublebarsep)],
                        **kwargs,
                    )
                )
                self.segments.append(
