...
class TimingDiagram(Element):
    ...
    def __init__(self, waved: dict, **kwargs):
        ...
        clipbox = BBox(
            0, self.yheight + 0.05, periods * self.yheight * 2 * self.hscale, -height
        )
        ...
        if foot:
            ...
            if "text" in foot:
                ...
                self.label(
                    foot["text"],
                    loc="bottom",
                    ofst=(0, 0 if tk is None else -self.ygap * 2),
                )
        ...
        for p in range(periods + 1):
            self.segments.append(
                Segment(
                    [
                        (
                            p * 2 * self.yheight * self.hscale,
                            self.yheight + self.ygap / 2,
                        ),
    ...
    def _drawedges(self):
        ...
        for edge in edges:
            ...
            if mode == "-":  # Straight line
                ...
                p0 = Point(
                    (p0.x + chrrad * math.cos(th0), p0.y + chrrad * math.sin(th0))
                )
                pn = Point(
                    (pn.x - chrrad * math.cos(th0), pn.y - chrrad * math.sin(th0))
                )
                self.segments.append(
                    Segment([pn, p0], lw=1, ls=ls, color=color, arrow=arrow, zorder=3)
                )
                ...
                self.segments.append(
                    Segment(
                        (p0, p1, pn), lw=1, ls=ls, color=color, zorder=3, arrow=arrow
                    )
                )
                ...
                dx = p3.x - p0.x
                p1 = p0 + Point((dx * 0.6, 0))
                p2 = p3 - Point((dx * 0.6, 0))
                self.segments.append(
                    SegmentBezier(
                        [p0, p1, p2, p3],
                        lw=1,
                ...
                pn = Point(
                    (pn.x - chrrad * math.cos(th0), pn.y + chrrad * math.sin(th2))
                )
                self.segments.append(
                    SegmentBezier(
                        [p0, p1, pn], lw=1, ls=ls, color=color, arrow=arrow, zorder=3
                ...
                pn = Point(
                    (pn.x + chrrad * math.cos(th2), pn.y + chrrad * math.sin(th2))
                )
                self.segments.append(
                    SegmentBezier(
                        [p0, p1, pn], lw=1, ls=ls, color=color, arrow=arrow, zorder=3
    ...
    def _drawgroups(self, signals, labelwidth):
        ...
        for label in getlabels(signals):
            ...
            ybot = (
                ytop
                - label.height * (self.yheight + self.ygap)
                + self.ygap / 2
                + edgelen
            )
            ycenter = (ytop + ybot) / 2
            ...
            self.segments.append(
                Segment(
                    [
                        (xval + edgelen, ybot - edgelen),
                        (xval, ybot),
                        (xval, ytop),
                        (xval + edgelen, ytop + edgelen),
                    ],
                    color=self.namecolor,
                    lw=1,
                )
