...
class Square(Element):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment([(0, 0), (0, 0.5), (1, 0.5), (1, -0.5), (0, -0.5), (0, 0)])
        )
...
class Sum(Circle):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.segments.append(Segment([(0.5, 0.2), (0.5, -0.2)]))
        self.segments.append(Segment([(0.3, 0), (0.7, 0)]))
...
class SumSigma(Circle):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.segments.append(
            SegmentText(label=r"$\Sigma$", pos=(0.45, 0), align=("center", "center"))
        )
...
class Mixer(Circle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if N:
            self.segments.append(
                SegmentText((rad, rad / 2 + 0.01), N, font=font, fontsize=fontsize)
            )
        if S:
            self.segments.append(
                SegmentText((rad, -rad / 2 - 0.065), S, font=font, fontsize=fontsize)
            )
        if E:
            self.segments.append(
                SegmentText((rad * 1.5 + 0.05, 0), E, font=font, fontsize=fontsize)
            )
        if W:
            self.segments.append(
                SegmentText((rad / 2 - 0.05, 0), W, font=font, fontsize=fontsize)
            )
...
class Speaker(Element):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(0, 0), (0, 0.25), (0.25, 0.25), (0.25, -0.25), (0, -0.25), (0, 0)]
            )
        )
        self.segments.append(
            Segment(
                [(0.25, 0.25), (0.5, 0.5), (0.5, -0.5), (0.25, -0.25), (0.25, 0.25)]
            )
        )
...
class Amp(Element):
    ...
    def __init__(self, **kwargs):
        ...
        ampl = 0.75
        self.segments.append(
            Segment([(0, 0), (0, -amph / 2), (ampl, 0), (0, amph / 2), (0, 0)])
        )
...
def _makesine() -> Sequence[XY]:
    ...
    sinx = [x / math.pi * 0.3 + 0.5 for x in sinx]
...
class Filter(Square):
    ...
    def __init__(self, response: Optional[FilterType] = None, **kwargs):
        super().__init__(**kwargs)
        path = _makesine()
        path1 = [Point((p[0], p[1] + 0.25)) for p in path]
        path2 = [Point((p[0], p[1] - 0.25)) for p in path]
        self.segments.append(Segment(path))
        ...
        if response:
            if response.lower() in ["bp", "lp"]:
                ...
                self.segments.append(Segment([(0.45, 0.17), (0.55, 0.33)]))
            if response.lower() in ["bp", "hp"]:
                # Slash through low f
                self.segments.append(Segment([(0.45, -0.33), (0.55, -0.17)]))
            if response.lower() in ["lp", "hp", "notch"]:
                # Slash through mid f
                self.segments.append(Segment([(0.45, -0.08), (0.55, 0.08)]))
...
class Adc(Element):
    ...
    def __init__(self, **kwargs):
        ...
        self.segments.append(
            Segment(
                [(0, 0), (0.22, 0.5), (1.4, 0.5), (1.4, -0.5), (0.22, -0.5), (0, 0)]
            )
        )
        self.elmparams["lblloc"] = "center"
        self.elmparams["lblofst"] = 0
...
class Demod(Square):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.segments.append(Segment([(0.15, 0), (0.3, 0)]))
        self.segments.append(
            Segment([(0.3, 0.25), (0.7, 0), (0.3, -0.25), (0.3, 0.25)])
        )
        self.segments.append(Segment([(0.7, 0.25), (0.7, -0.25)]))
        self.segments.append(Segment([(0.7, 0), (0.85, 0)]))
